      ...
      <div className="mb-8">
        ...
        <div className="flex gap-4 mb-2">
          <input 
            ...
            className="flex-1 bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-black focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
          />
          <CldUploadWidget 
            uploadPreset="ml_default"
            options={{ resourceType: "video" }}
            onSuccess={(result: any) => setHeroVideoUrl(result.info.secure_url)}
          >
            {({ open }) => (
              <button 
                type="button" 
                onClick={() => open()}
                className="bg-gray-100 text-black px-4 py-3 rounded-xl font-bold text-sm hover:bg-gray-200 transition-colors whitespace-nowrap"
              >
                Subir Video
              </button>
            )}
        ...
        <p className="text-sm text-gray-400">Puedes pegar una URL directa o subir un archivo MP4 desde tu computadora.</p>
      </div>
